...
# sort(a[i,1:12])
# order(a[i,1:12])
# # 返回了排序后原始样品的编号，样本顺序被打乱，而不是样本的丰度顺序，如wt1丰度排序为第7，显示为wt5的位置3
# # 我们想要的是原来样本排序后的顺序编号
# a[i,1:12]
# sort(a[i,1:12])
# match(colnames(a[i,1:12]), colnames(sort(a[i,1:12])))
# # 返回原始样本，在排序中的位置，为原样本的属性
...
ann_all2$ring=as.character(ann_all2$ring)
ste6$ring=as.character(ste6$ring)
